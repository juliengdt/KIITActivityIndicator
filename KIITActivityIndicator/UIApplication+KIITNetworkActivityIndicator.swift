...
extension UIApplication {
	...
	private struct AssociatedKeys {
		static var indicatorWindowKey = "KIITActivityIndicatorWindowKey"
	}
	...
	@objc func ft_setNetworkActivityIndicatorVisible(visible: Bool) {
		...
		if visible {
			if indicatorWindow == nil {
				...
				let indicator = KIITActivityIndicator(frame: CGRect(x: indicatorWindow!.frame.width - 74, y: 6, width: 44, height: 4))
				indicator.hidesWhenStopped = false
				indicator.startAnimating()
				indicatorWindow?.addSubview(indicator)
			}
		}
		guard let indicator = indicatorWindow?.subviews.first as? KIITActivityIndicator else {return}
